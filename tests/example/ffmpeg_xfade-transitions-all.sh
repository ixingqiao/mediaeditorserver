#!/bin/bash
# Description: This script concatenates multiple MP4 files with different transitions and outputs each with the transition name.
...
# run all transitions then check the results
...
# Generate filter_complex string for a single transition
function generate_filter_complex() {
    ...
    for ((i=0; i<$line; ++i)); do
        vfstr+="[v${i}11][v$((i+1))01]xfade=duration=${interval}:transition=${1}[vt${i}];"
    done
...
# Iterate over each transition and create the output file
for transition in "${transitions[@]}"; do
    output_file="$output_dir/${transition}.mp4"
    log "Processing transition: $transition"
    
    generate_filter_complex "$transition"
